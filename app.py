...
class MusicApp(App):
    ...
    DEFAULT_CSS = """
      ...
      ListView > ListItem.--highlight {
        background: #17283e 20%;
      }

      ListView:focus > ListItem.--highlight {
        background: #17283e 40%;
      }
    """
        ...
        with Horizontal():
            ...
            with Vertical(id="main-panel"):
                ...
                with Vertical(id="now-playing"):
                    with Horizontal():
                        with Vertical(id="now-playing-info"):
                            yield Label(
                                "🎵 Nothing playing yet", id="now-playing-title"
                            )
    ...
    def on_mount(self) -> None:
        ...
        self.player.on_track_change(lambda t: self._safe_post(TrackChanged(t)))

        self.player.on_state_change(lambda s: self._safe_post(StateChanged()))
    ...
    def _populate_artists(self) -> None:
        ...
        for index, artist in enumerate(self.library.artists):
            artist_list.append(ListItem(Label(artist.name), id=f"artist-{index}"))
    ...
    def _update_now_playing(self, track: Track) -> None:
        ...
        self.query_one("#now-playing-artist-album", Label).update(
    ...
    def _update_progress(self) -> None:
        """
        Called every 500ms by set_interval.
